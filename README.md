...
The binary was decompiled using a disassembler, and the resulting pseudocode was cleaned up and corrected with the help of an AI assistant. The bulk of the work — resolving magic numbers, naming data structures, and restoring x64 compatibility — is done. The reconstructed source compiles and runs on modern Windows, with support for dark mode. There is still room to improve: the code can be made more idiomatic.
